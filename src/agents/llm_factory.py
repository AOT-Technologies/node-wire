...
Supported providers (set via LLM_PROVIDER env var):
  ...
  openrouter            — OpenRouter OpenAI-compatible (e.g. poolside/laguna-s-2.1:free)
"""
...
DEFAULT_OPENROUTER_MODEL = DEFAULT_OPENROUTER_MODELS[0]
...
OPENROUTER_TOOLS_NOTE = (
    "OpenRouter free models log prompts and outputs and must not be used with confidential "
    "or personal data. Tool calling may be limited; switch back to Groq if tools fail."
)
...
def resolve_openrouter_models() -> List[str]:
    """Playground catalog: ``OPENROUTER_MODELS``, else ``OPENROUTER_MODEL``, else built-in defaults."""
    listed = [m.strip() for m in os.environ.get("OPENROUTER_MODELS", "").split(",") if m.strip()]
    if listed:
        return listed
...
class LLMProviderFactory:
    ...
    Environment variables:
        ...
        OPENROUTER_API_KEY / OPENROUTER_BASE_URL / OPENROUTER_MODEL / OPENROUTER_MODELS
    """
    ...
    def create(cls, provider: str, **kwargs: Any) -> BaseLLMProvider:
        ...
        if provider == "groq":
        ...
        elif provider in ("openai", "nvidia", "ollama", "openrouter"):
            ...
            supported = ["groq", "openai", "gemini", "anthropic", "nvidia", "ollama", "openrouter"]
            raise ValueError(
    ...
    def create_from_env(cls) -> BaseLLMProvider:
        ...
        if provider == "groq":
            ...
            kwargs["model"] = (
                os.environ.get("OLLAMA_MODEL", DEFAULT_OLLAMA_MODEL).strip() or DEFAULT_OLLAMA_MODEL
            )
            ...
            kwargs["base_url"] = resolve_openrouter_base_url()
    ...
    def list_playground_options(cls) -> Dict[str, Any]:
        ...
        if openrouter_key:
            for openrouter_model in resolve_openrouter_models():
                openrouter_id = f"openrouter/{openrouter_model}"
                options.append(
                    {
                        "id": openrouter_id,
                        "label": openrouter_id,
                        ...
                        "tools_note": OPENROUTER_TOOLS_NOTE,
                        "source": "env",
                    }
                )
