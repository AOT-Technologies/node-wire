...
`build_manifest(connectors)` is the single source of truth for both bindings. It returns one entry per manifest action (explicit or generated from **`action_specs`**):
...
[
  {
    "connector_id": "google_drive",
    "action": "files.list",
    "input_schema": { ... },   # JSON Schema from FilesListOperation
    "output_schema": { ... },  # JSON Schema from GoogleDriveOperationOutput
  },
  {
    "connector_id": "google_drive",
    "action": "files.upload",
