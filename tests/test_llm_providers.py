...
def test_llm_factory_create_openrouter_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("LLM_PROVIDER", "openrouter")
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-or-v1-test")
    monkeypatch.setenv("OPENROUTER_MODEL", "poolside/laguna-s-2.1:free")
    with patch("agents.providers.openai_provider.OpenAI") as ctor:
        provider = LLMProviderFactory.create_from_env()
    from agents.providers.openai_provider import OpenAIProvider

    assert isinstance(provider, OpenAIProvider)
    assert provider._model == "poolside/laguna-s-2.1:free"
    ctor.assert_called_once_with(
...
def test_llm_factory_create_from_option_ollama(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("agents.providers.openai_provider.OpenAI") as ctor2:
        LLMProviderFactory.create_from_option(
            "ollama/qwen2.5:7b",
...
def test_llm_factory_list_playground_options_includes_openrouter(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    row = catalog["options"][0]
    assert row["provider"] == "openrouter"
    assert row["model"] == "poolside/laguna-s-2.1:free"
    assert row["tools_note"]
    assert "confidential" in row["tools_note"].lower()


def test_llm_factory_list_playground_options_openrouter_models(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("GROQ_API_KEY", raising=False)
    monkeypatch.delenv("NVIDIA_API_KEY", raising=False)
...
def test_parse_llm_option_and_tool_heuristic() -> None:
    ...
    assert parse_llm_option("openrouter/poolside/laguna-s-2.1:free") == (
        "openrouter",
